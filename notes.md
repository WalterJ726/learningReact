...
## 公司遇到的问题
1. 把前端代码打包放到django中，开放了80端口，访问的时候前端可以跳转，但是一刷新，django没有注册这个路由，显示跳转不成功。（但是前端如果使用了nginx之后，刷新那个页面就没有任何问题）
解决方案？还在查资料中：前端有着nginx作反向代理，后端开放另外的端口给前端申请访问数据
2. url的设计不能是createTask这个带驼峰的样子，如果应该改成tasks/create这样
3. 前后端交互需要共用一套url吗？比如说/api/tasks。我暂时能想到这样的好处就是前端某个url显示出问题的时候，能立马在后端找到对应的函数，因为url是对应的。
4. 跨域请求，真正跨域请求的例子是怎么样的
5. 反向代理，我使用nginx加react。访问/home不需要访问后端，会反向代理到后端的url嘛？我一直以为我使用/tasks/，后端可以直接对这个进行相应返回数据就是反向代理的作用